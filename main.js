...
(function() {
  var addFrame, buildStyle, condStyle, defaultStyle, deleteLeft, deleteRight, deleteUnder, drawSelection, flowLeft, flowRight, indexAfter, indexBefore, indexBottom, indexTop, labelStyle, liftLeft, liftRight, splitNode, stepLeft, stepRight;
  ...
  stepLeft = function(cursor) {
    ...
    if (isList(node)) {
      ...
      if (index > 0) {
        ...
        _ref = cursor = indexBefore(node), node = _ref.node, index = _ref.index;
        if (isList(node) && index === 0 && (node.parent != null)) {
          cursor = indexBefore(node);
        }
      }
    }
  ...
  stepRight = function(cursor) {
    ...
    if (isList(node)) {
      ...
      if (index < node.length) {
        ...
        _ref = cursor = indexAfter(node), node = _ref.node, index = _ref.index;
        if (isList(node) && index === node.length && (node.parent != null)) {
          cursor = indexAfter(node);
        }
      }
    }
